    ...
    "L'architecture": [
        {
            ...
            correct: 2,
            explanation: "Le modèle Model-View-Controller (MVC) est un pattern architectural utilisé dans le développement d'applications, notamment pour les interfaces utilisateur.\nIl divise une application en trois composants principaux.\n\t- Le modèle (Model) représente la logique métier et les données de l'application. Il gère la récupération, la manipulation et la validation des données, sans contenir d'informations sur la manière dont les données sont affichées à l'utilisateur.\n\t- La vue (View) représente l'interface utilisateur. Elle affiche les données provenant du modèle et réagit aux changements dans le modèle pour mettre à jour l'affichage. - Le contrôleur (Controller) agit comme un intermédiaire entre le modèle et la vue. Il gère les entrées de l'utilisateur et les commandes, met à jour le modèle en fonction des actions de l'utilisateur et notifie la vue des changements. En résumé, le MVC permet de séparer les préoccupations dans une application, facilitant ainsi la maintenance, la testabilité et la réutilisabilité du code."
        },
        {
        ...
        },
        {
            ...
            correct: 2,
            explanation: "Le Model-View-ViewModel (MVVM) est un pattern architectural qui sépare le développement de l'interface graphique (la Vue) des règles métier et de la logique de présentation (le ViewModel) et des données (le Modèle).\n\t- Le Modèle représente les données et la logique métier de l'application, indépendamment de l'interface utilisateur.\n\t- La Vue est responsable de définir la structure, la mise en page et l'apparence de ce que l'utilisateur voit à l'écran. Dans MVVM, la Vue est idéalement passive et ne contient pas de logique métier.\n\t- Le ViewModel agit comme un intermédiaire entre le Modèle et la Vue.\nIl encapsule la logique de présentation et l'état de la Vue.\nLe ViewModel expose des propriétés et des commandes auxquelles la Vue peut se lier.\n\nMVVM utilise un mécanisme de liaison de données (data binding) pour connecter le ViewModel à la Vue, ce qui permet une séparation claire des préoccupations et facilite les tests unitaires de la logique de présentation.\n\n<b>Exemple en Python</b>: \n# Modèle\nclass User:\n    def __init__(self, name):\n        self.name = name\n\n# ViewModel\nclass UserViewModel:\n    def __init__(self, user):\n        self._user = user\n\n    def get_display_name(self):\n        return self._user.name.upper()\n\n# Vue (simulée)\nclass UserView:\n    def __init__(self, view_model):\n        self.view_model = view_model\n\n    def display_user(self):\n        print(f\"Nom: {self.view_model.get_display_name()}\")\n\n# Utilisation\nuser = User(\"Alice\")\nview_model = UserViewModel(user)\nview = UserView(view_model)\n\nview.display_user()"
        },
        {
